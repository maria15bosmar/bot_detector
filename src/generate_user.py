...
idiomas = {"en": [nltk.stem.snowball.EnglishStemmer(), "english"], "es": [nltk.stem.snowball.SpanishStemmer(), "spanish"]}
...
def get_user_data(user):
    ...
    languages = tweets["language"].values
    # Tokenizar, stemmear, filtrar stop-words.
    for ind, content in enumerate(contents.values):
        tk = nltk.word_tokenize(content)
        lan = languages[ind]
        filtered_sentence = [w for w in tk if not w.lower() in sw.words(idiomas[lan][1])]
        stemmer = idiomas[lan][0]
